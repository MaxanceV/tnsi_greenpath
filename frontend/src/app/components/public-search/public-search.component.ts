...
@Component({
  ...
  template: `
    <div class="search-page">
        ...
        <div class="hero-inner">
          <div class="logo-row">
            <svg width="32" height="32" viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg">
              ...
              <path d="M20 10 C14 14, 11 20, 14 27 C17 34, 26 34, 29 27 C32 20, 29 14, 23 11"
                    stroke="white" stroke-width="2.5" fill="none" stroke-linecap="round"/>
              <circle cx="20" cy="20" r="4" fill="white"/>
            </svg>
            <span class="logo-name">GreenPath</span>
          </div>
          <h1>Traçabilité des produits</h1>
          <p class="hero-sub">Recherchez un produit par nom, GTIN, fournisseur ou lieu d'origine</p>
          ...
          <div class="search-bar">
            ...
            <input
              ...
              autofocus
            />
            <span class="search-clear" *ngIf="query" (click)="clear()">✕</span>
          </div>
        </div>
      </div>
      ...
      <div class="results-wrap">
        ...
        <p class="status-msg" *ngIf="query.length > 0 && query.length < 2">
          Continuez à taper...
        </p>
        ...
        <div class="results-grid">
          ...
          >
            ...
            <div class="card-meta">
              ...
              <span class="steps-tag">{{ p.steps.length }} étape{{ p.steps.length > 1 ? 's' : '' }}</span>
              <span class="co2-tag" *ngIf="p.total_co2_kg && p.total_co2_kg > 0">
                {{ p.total_co2_kg | number:'1.0-2' }} kg CO₂
              </span>
            </div>

            <div class="step-pills" *ngIf="p.steps.length">
              <span
                *ngFor="let s of p.steps.slice(0, 4)"
                class="step-pill"
                [class]="'type-' + s.step_type"
              >{{ stepTypeLabels[s.step_type] }}</span>
  ...
  styles: [`
    ...
    .logo-name { color: white; font-size: 1.1rem; font-weight: 700; letter-spacing: -0.01em; }

    .hero h1 { color: white; font-size: 2rem; font-weight: 800; margin: 0 0 8px; letter-spacing: -0.02em; }
    ...
    .search-icon { position: absolute; left: 14px; color: #9ca3af; flex-shrink: 0; }
    .search-input {
      width: 100%; border: none; outline: none; background: transparent;
      padding: 16px 40px 16px 44px; font-size: 1rem; border-radius: 12px;
      font-family: inherit; color: #1f2937;
    }
    .search-clear {
      position: absolute; right: 14px; color: #9ca3af; cursor: pointer;
    ...
    .gtin-badge {
      font-size: 0.7rem; font-weight: 600; background: #f0fdf4; color: #065f46;
      border: 1px solid #bbf7d0; border-radius: 6px; padding: 2px 8px; white-space: nowrap;
    }
